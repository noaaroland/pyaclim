...
1. **regions3.ipynb** -  is the entire process. It shows how to find the centroids from the shape file, create data arrays from the result, add those data arrays to the data set read from the file and write the file back out. In order for ERDDAP to understand it you have to make "flat" very similar to a continuous ragged array DSG by putting it in a dataframe and resetting the multilevel index.
